...
public class ItemDAO extends GenericDAO<ItemBean> {
	...
	public ItemBean[] getItems() throws RollbackException {
		...
		// Calls GenericDAO's match() method.  You can provide parameters
	    // to match to constrain which beans are returned.  With no
	    // parameters, all beans in the table are returned.
		ItemBean[] items = match();
		
		Arrays.sort(items, (ItemBean i1, ItemBean i2) -> i1.getPosition()
				- i2.getPosition());
