/*
 * Copyright (c) 2005-2016 Jeffrey L. Eppinger.  All Rights Reserved.
...
public class AccessController extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("login") == null) {
            // We're not logged in, so send the login form (which comes back using POST)
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (code == null || code.length() == 0) {
            ...
            sendLoginPage(response, afterLogin, makeCookie,
                    "The access code is required.  Try again.");
            return;
        }
        ...
        if (!code.equals(accessCode)) {
            ...
            sendLoginPage(response, afterLogin, makeCookie,
                    "The access code is invalid.  Try again.");
            return;
        }
    ...
    /*
     * If we're not already logged in, check the cookie called "my-cookie". If
     * the cookie is present and it contains the correct value (cookieCode from
     * the web.xml file) then log in the user. We track whether a user is logged
     * in by the presence of the "login" session variable.
     */
    private void loginWithCookie(HttpServletRequest request) {
        ...
        for (Cookie cookie : cookies) {
            if (cookie.getName().equals(cookieName)
                    && cookie.getValue().equals(cookieCode)) {
                request.getSession().setAttribute("login", true);
                return; // Found it. No point in checking other cookies.
            }
        }
    }
    ...
    private void sendTextFile(HttpServletRequest request, HttpServletResponse response, 
    				String reqInfo, String fileName)
            throws IOException {
    ...
    private void sendBinaryFile(HttpServletRequest request, HttpServletResponse response,
    				String reqInfo, String fileName)
            throws IOException {
        if (fileName.contains("/") || fileName.contains("\\")) {
        ...
        try {
            ...
            if (suffix.equals(".gif")) {
                ...
                logRequest(request, "sendBinaryFile: req=" + reqInfo
                        + ": *** UNSUPPORTED_MEDIA_TYPE *** file=" + fileName);
            ...
            logRequest(request, "sendImage: req=" + reqInfo
                    + ": IOException: file=" + fileName + ": " + e.getMessage());
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    private void sendLoginPage(HttpServletResponse response, String pageRequested,
                String makeCookie, String errorMessage)
            throws IOException {
        response.setContentType("text/html");
        PrintWriter out = response.getWriter();
        ...
        if (errorMessage != null) {
            out.println("<div style=\"font-size: large; color: red\">");
        ...
        out.println("        <input type=\"password\" name=\"code\" autfocus>");
