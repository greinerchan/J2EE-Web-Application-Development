...
public class UpdateEntryAction extends Action {
    ...
    public String performPost(HttpServletRequest request) {
        try {
            ...
            EntryForm form = new EntryForm(request);
    ...
    private String createCase(HttpServletRequest request, EntryForm form) throws RollbackException {
        ...
        request.setAttribute("numEntries", entryDAO.getCount()); // It's now one
                                                                 // bigger
    ...
    private String updateCase(HttpServletRequest request, EntryForm form) throws RollbackException {
        ...
        try {
            ...
            request.setAttribute("message",
                    "Updated record for " + newEntry.getLastName() + ", " + newEntry.getFirstNames());
            return "display-entry.jsp";
        } catch (RollbackException e) {
            // Perhaps someone else was edited this entry. Try looking up the entry. If we
            // can
            // find it let's redisplay the entry for editing along with the DAOException's
            // message.
    ...
    private void logIfChanged(HttpServletRequest request, String fieldName, java.sql.Date oldV, java.sql.Date newV)
        ...
        logDAO.write(request, "     " + fieldName + ": old=" + (oldV == null ? "null" : sdf.format(oldV)) + ", new="
                + (newV == null ? "null" : sdf.format(newV)));
    }

    private void logIfChanged(HttpServletRequest request, String fieldName, String oldV, String newV)
            throws RollbackException {
        // Null Strings not permitted in this app, but we check in case someone
